...
namespace velodyne_driver
{
 ...
 Input::Input(ros::NodeHandle private_nh, uint16_t port):
  ...
  //  u_char gpsBuffer[72],time_stamp[4];
    ...
    while (true)
      {
        ...
        if ((res = pcap_next_ex(pcap_, &header, &pkt_data)) >= 0)
          ...
          // memcpy(&gpsBuffer, pkt_data+248, 72);
           //memcpy(&time_stamp, pkt_data+240, 4);
       //ROS_INFO("Buffer: x %x %x %x",pkt_data[42],pkt_data[43],pkt_data[248]);
