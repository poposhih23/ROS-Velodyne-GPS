                               ...
                               ros::NodeHandle private_nh)
{
  ...
  private_nh.param("port", udp_port, (int) POSITION_PORT_NUMBER);
ROS_INFO("Port=%d",udp_port);
...
int pkgCount=0,header=0,rec=0;
  ...
  // reading and publishing scans as fast as possible.
    ...
    {
    //ROS_INFO("Diver.cc : %d:%d", config_.npackets,i);
      while (true)
        {
   ...
   //copy GPS messages
          if(scan->packets[i].data[0]==0){
              for(int j=0;j<72;j++){

                         gpsBuffer[j]=scan->packets[i].data[206+j];
                        //cerr<<gpsBuffer[j];
...
//ROS_INFO("input:%d header:%d",i,header);
//put gps message(72 bytes) after the pkt_data
if(scan->packets[i].data[0]==0){
         for (int jj=0 ; jj<pkgCount ;jj++){

               for(int j=0;j<72;j++){
                          
                      scan->packets[header+jj].data[1206+j]= gpsBuffer[j];

                      //cerr<<scan->packets[header+jj].data[1206+j];
                   }//cerr<<endl;
          }
